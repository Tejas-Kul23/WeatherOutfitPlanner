...
class LoginUser(tk.Frame):
    def __init__(self, parent, controller) -> None:
        tk.Frame.__init__(self, parent)
        self.controller = controller
        self.controller.title('Login User')
        ...
        # Implement Login Button to run method login()
        self.login_response = ttk.Label(self, text='')
        self.login_response.grid(row=4, column=0, padx=10)

        self.login_btn = ttk.Button(self, text='Login', command=self.login)
        self.login_btn.grid(row=5, column=0, padx=10)
